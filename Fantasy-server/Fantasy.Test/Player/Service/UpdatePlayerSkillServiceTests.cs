...
public class UpdatePlayerSkillServiceTests
{
    public class 정상_요청일_때
    {
        ...
        private readonly UpdatePlayerSkillService _sut;
        private readonly List<SkillChangeItem> _skills = [new(1, true)];
        ...
        public 정상_요청일_때()
        {
            ...
            _sut = new UpdatePlayerSkillService(
                _playerRepository, _playerSkillRepository, _playerRedisRepository, _currentUserProvider);
        }

        [Fact]
        public async Task 스킬_Upsert가_호출된다()
        {
            var request = new UpdatePlayerSkillRequest(JobType.Warrior, _skills);

            await _sut.ExecuteAsync(request);

            await _playerSkillRepository.Received(1).UpsertRangeAsync(Arg.Any<long>(), _skills);
        }
    ...
    public class 플레이어가_존재하지_않을_때
    {
        ...
        {
            var request = new UpdatePlayerSkillRequest(JobType.Warrior, [new(1, true)]);
